package ise1005.edu.fpt.vn.myrestaurant.config;

/**
 * Created by DungNA on 10/20/17.
 */

public class Constants {
    public static final String API_URL = "http://192.168.1.32:8888/";
    public static final String PUSHER_KEY = "ff3ea1c0573eb96b7ae2";
    public static final String PUSHER_CHANNEL = "ISE1005";
    public static final String PUSHER_EVENT_FOR_MANAGER = "MYRESTAURANT_MANAGER";
    public static final String PUSHER_EVENT_FOR_COOKER = "MYRESTAURANT_COOKER";
    public static final String PUSHER_EVENT_FOR_STAFF = "MYRESTAURANT_STAFF";
}
