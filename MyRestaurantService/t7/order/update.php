...
if(isset($_POST['id'])  && isset($_POST['order_detail']) && count($_POST['order_detail'])>0){
    ...
    if($id>0){
        foreach($_POST['order_detail'] as $order_detail){
            ...
            if($or_id>0){
                $sql = "UPDATE `order_detail` SET `quantity` = {$order_detail['quantity']} WHERE `id`={$or_id}";
                $mysqli->query($sql);                
                $ids[] = $or_id;
            }else{
                $sql = "INSERT INTO `order_detail` (`order_id`, `product_id`, `price`, `quantity`, `note`) VALUES ({$or_id},{$order_detail['product_id']},{$order_detail['price']},{$order_detail['quantity']},'{$order_detail['note']}')";
                $mysqli->query($sql);                
                $ids[] = $mysqli->insert_id;
            }
            if(!$mysqli->connect_errno){
                $mysqli->query("DELETE FROM `order_detail` WHERE `order_id` = {$id} AND `id` NOT IN (".implode(',', $ids).")");
                $outp['hasErr'] = false;
